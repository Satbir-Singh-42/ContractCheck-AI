...
import { apiCreatePaymentOrder, apiVerifyPayment } from '../../lib/api';
...
export function PaymentPage() {
  ...
  const handlePay = async () => {
    if (selectedMethod !== 'razorpay') {
      alert('Only Razorpay is fully integrated. Please select Razorpay.');
      return;
    }
    setLoading(true);
    try {
      const order = await apiCreatePaymentOrder(selectedPlan as 'pro_monthly' | 'pro_annual');
      const options = {
        key: order.key_id,
        ...
        handler: async function (response: any) {
          try {
            await apiVerifyPayment(
              ...
              selectedPlan as 'pro_monthly' | 'pro_annual'
            );
            navigate('/success');
          } catch (err) {
